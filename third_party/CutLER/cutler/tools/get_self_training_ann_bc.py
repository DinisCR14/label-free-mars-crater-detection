...
if __name__ == "__main__":
    ...
    for k, anns_pseudo in tqdm.tqdm(pseudo_image_to_anns.items()):
        # Separate pseudo-labels fully inside the image vs out-of-bounds
        filtered_anns_pseudo = []
        for ann in anns_pseudo:
            x, y, w, h = ann['bbox']
            if x >= 0 and y >= 0 and (x + w) <= image_width and (y + h) <= image_height:
                filtered_anns_pseudo.append(ann)
            else:
                # Out-of-bounds boxes are kept untouched
                merged_anns.append(ann)
        
        if not filtered_anns_pseudo:
            # No in-bounds pseudo-labels to consider; skip IoU computation
            continue

        # Compute masks for in-bounds pseudo-labels
        masks = []
        for ann in filtered_anns_pseudo:
            segm = ann['segmentation']
            mask = cocosegm2mask(segm, segm['size'][0], segm['size'][1])
            masks.append(torch.from_numpy(mask))
        pseudo_masks = torch.stack(masks, dim=0).cuda()
        del masks
        num_pseudo += len(filtered_anns_pseudo)

        # Get predictions for this image
        try:
            anns_pred = pred_image_to_anns[k]
        except KeyError:
            # No predictions for this image, keep in-bounds pseudo-labels
            merged_anns += filtered_anns_pseudo
            continue

        # Compute masks for predictions
        masks = []
        ...
        # Compute IoU and select pseudo-labels not replaced
        try:
            ious = BatchIoU(pseudo_masks, pred_masks)
            iou_max, _ = ious.max(dim=1)
            selected_index = (iou_max < 0.5).nonzero()
            selected_pseudo = [filtered_anns_pseudo[i] for i in selected_index]
            merged_anns += anns_pred + selected_pseudo
        except:
            # fallback if IoU computation fails
            merged_anns += filtered_anns_pseudo
